...
fn derive_kek(secret: &str, salt_str: &str) -> [u8; 32] {
...
pub fn init_keychain(password: &str) -> Result<(String, MasterKey)> {
    ...
    // 2. Prepare Password Slot
    let pass_salt = SaltString::generate(&mut OsRng).as_str().to_string();
    let pass_kek = derive_kek(password, &pass_salt);
    ...
    let rec_kek = derive_kek(&recovery_code, &rec_salt);
...
pub fn unlock_keychain(password: &str) -> Result<MasterKey> {
    ...
    let kek = derive_kek(password, &store.password_salt);
...
pub fn recover_with_code(recovery_code: &str, new_password: &str) -> Result<MasterKey> {
    ...
    let rec_kek = derive_kek(recovery_code, &store.recovery_salt);
    let cipher_rec = Aes256Gcm::new_from_slice(&rec_kek).unwrap();
    ...
    let new_pass_kek = derive_kek(new_password, &new_pass_salt);
